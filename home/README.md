...
The ID410 read-only RAM trace is disabled by default; `ID410_TRACE_ENABLED = "true"` enables it on either port for diagnostics. The ID498 snapshot optimization and five-second polling run only for software ID 498. Other supported washers and dryers use their profile's normal `Operation` properties at `DEVICE_PUBLISH_INTERVAL` (default 60 seconds). If one property read fails, the gateway resynchronizes and retries missing properties in fresh sessions, then publishes every successfully read value. Unreadable properties are marked `unknown` instead of hiding the other entities. If no properties can be read, the polling cycle is retried once. These device-specific features do not imply equivalent addresses or semantics on another software ID.
