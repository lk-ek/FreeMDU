...
const BACKLOG_LINES: usize = 64;

static LOG_CHANNEL: Channel<CriticalSectionRawMutex, LogLine, BACKLOG_LINES> = Channel::new();
...
impl Log for NetLogger {
    ...
    fn log(&self, record: &Record<'_>) {
        ...
        // must never block the firmware, so when the backlog is full the
        // oldest line is dropped in favour of the newest one.
        let mut line = LogLine::new();
        let _ = writeln!(&mut line, "{} - {}", record.level(), record.args());

        if let Err(embassy_sync::channel::TrySendError::Full(line)) = LOG_CHANNEL.try_send(line) {
